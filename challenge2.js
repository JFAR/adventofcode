...
function updatePosition(position, dir, displacement) {
  if (displacement == 0) {
    ...
    var newPosition = [position[0] + dir[0], position[1] + dir[1]];
    if (newPosition in placesVisited) {
      console.log(abs(newPosition[0])+abs(newPosition[1]));
    }
    placesVisited[newPosition] = true;
    return updatePosition(newPosition, dir, displacement - 1);
  }
}
